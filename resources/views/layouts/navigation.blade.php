    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6 gap-3">
                <a href="{{ route('notifications.index') }}" class="relative inline-flex items-center p-2 text-gray-500 hover:text-gray-700" title="{{ __('Notifications') }}">
                    ...
                    @if (($unread = Auth::user()->unreadNotifications->count()) > 0)
                        <span class="absolute -top-0.5 -right-0.5 inline-flex items-center justify-center min-w-[1.1rem] h-4 px-1 rounded-full bg-red-500 text-[10px] font-semibold text-white">{{ $unread > 9 ? '9+' : $unread }}</span>
                    @endif
                </a>
